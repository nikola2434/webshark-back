...
const SHARKD_SOCKET = "/tmp/sharkd.sock";
const CAPTURES_PATH = "/home/g10/captures/";
var sharkd_objects = {};
...
get_sharkd_cli = async function (capture) {
  ...
  if (socket_name in sharkd_objects) {
    ...
    // no socket for this capture existst, create new one
    ...
    if (capture !== "") {
      await send_req(
        { req: "load", file: capture },
        sharkd_objects[socket_name]
      );
      return sharkd_objects[socket_name];
    } else {
      return sharkd_objects[socket_name];
...
send_req = async function (request, sock) {
  ...
  if ("capture" in request) {
    if (request.capture.includes("..")) {
      return JSON.stringify({ err: 1, errstr: "Две точки" });
    }
    ...
    if (fs.existsSync(cap_file) === false) {
      return JSON.stringify({ err: 1, errstr: "Такого файла не существует" });
    }
  }
  ...
  async function _send_req_internal() {
    ...
    if (new_sock === null) {
      return JSON.stringify({
        err: 1,
        errstr: `cannot connect to sharkd using socket: ${SHARKD_SOCKET}`,
      });
    }
    try {
